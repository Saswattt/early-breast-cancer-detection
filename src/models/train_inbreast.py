...
DATA_DIR = os.path.join(BASE_DIR, 'data/processed/INbreast')
...
def train_model():
    ...
    history = model.fit(
        ...
        callbacks=[
            ...
            tf.keras.callbacks.ModelCheckpoint(
                os.path.join(model_dir, 'inbreast_model_best.h5'),
                save_best_only=True,
                monitor='val_accuracy'
            )
        ],
        verbose=1  # Make sure training progress is shown
    )
    ...
    # Save final model
    model.save(os.path.join(model_dir, 'inbreast_model_final.h5'))
    
    return model, history

if __name__ == "__main__":
    # Add this line to actually run the training
    train_model()
